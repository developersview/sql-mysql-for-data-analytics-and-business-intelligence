...
in the WITH clause of a common table expression whose top-level SELECT statement 
should combine all the data from the salaries_adjusted_for_inflation table 
with the common table expression data vertically, excluding duplicate rows from both tables.
*/
WITH cte AS(
SELECT 
    emp_no,
...
SELECT DISTINCT
    *
FROM
    salaries_adjusted_for_inflation 
